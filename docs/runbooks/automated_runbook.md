# SolentCloud Traffic Engineering: Automated CLI Runbook
...
```Bash
cd /traffic_shaping
ansible-playbook degrade_bt.yml
```
...
```Bash
cd /traffic_shaping
ansible-playbook degrade_vmo2.yml
```
...
```Bash
cd /traffic_shaping
ansible-playbook degrade_ixp_lon.yml
```
...
```Bash
cd /traffic_shaping
ansible-playbook degrade_ixp_man.yml
```
...
Eensure to `cd` to the traffic shaping directory before executing

| Target to Restore | Affected Router(s) | BGP Group | Command to Revert to Normal |
| :--- | :--- | :--- | :--- |
| **BT ISP** | Edge Router 1 | `EBGP-BT` | `ansible-playbook restore_bt.yml` |
| **VMO2 ISP** | Edge Router 2 | `EBGP-VMO2` | `ansible-playbook restore_vmo2.yml` |
| **London IXP** | Edge 1 & Edge 2 | `IXP-LON` | `ansible-playbook restore_ixp_lon.yml` |
| **Manchester IXP** | Edge 1 & Edge 2 | `IXP-MAN` | `ansible-playbook restore_ixp_man.yml` |